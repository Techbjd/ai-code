#!/usr/bin/env python3
"""
VEGFR2 Feature Comparison - Full Pipeline (Colab Ready)
======================================================
Tests ALL feature combinations:
- ML: MACCS only, Morgan only, Morgan+MACCS
- GNN: Graph only (no FP), Enriched (atom+Morgan+MACCS)

Shows WHY enriched graphs work better.
...
# %% [markdown]
# # VEGFR2 Feature Comparison Pipeline
# ## Comparing MACCS vs Morgan vs Enriched Graphs
# Shows the impact of different feature combinations on model performance.
# Runs on GPU (T4) in ~25-35 minutes.
...
# %%
# @title 6. PART A: ML Models with Different Features
...
# Define feature sets
feature_sets = {
    "MACCS": (X_train_maccs, X_val_maccs, X_test_maccs),
    "Morgan": (X_train_morgan, X_val_morgan, X_test_morgan),
    "Morgan+MACCS": (X_train_both, X_val_both, X_test_both),
}

ml_models = ["rf", "svm", "xgb"]

print("=" * 70)
print("PART A: ML MODELS WITH DIFFERENT FEATURES")
print("=" * 70)

for feat_name, (X_tr, X_vl, X_te) in feature_sets.items():
    print(f"\n--- Features: {feat_name} ({X_tr.shape[1]}-dim) ---")
    
    for ml_name in ml_models:
        model_name = f"ml_{ml_name}_{feat_name}"
        print(f"\n  Training {model_name}...")
        
        model = train_ml_model(ml_name, X_tr, y_train, seed=42)
...
# %%
# @title 7. PART B: GNN Models - Graph Only (No Fingerprints)
...
print("\n" + "=" * 70)
print("PART B: GNN MODELS - GRAPH ONLY (32-dim atom features, NO fingerprints)")
print("=" * 70)
...
# %%
# @title 8. PART C: GNN Models - Enriched Graphs (atom + Morgan + MACCS)
from torch_geometric.data import Data
from torch_geometric.loader import DataLoader
...
print("\n" + "=" * 70)
print("PART C: GNN MODELS - ENRICHED GRAPHS (2246-dim = atom + Morgan + MACCS)")
print("=" * 70)
...
# %%
# @title 9. PART D: Ensemble Models (GNN + ML)
...
print("\n" + "=" * 70)
print("PART D: ENSEMBLE MODELS (GNN embeddings + Morgan + MACCS + XGBoost/RF)")
print("=" * 70)
...
# %%
# @title 10. FINAL COMPARISON TABLE
print("\n" + "=" * 100)
print("VEGFR2 FEATURE COMPARISON - ALL RESULTS")
print("=" * 100)

# Group results by category
ml_results = {k: v for k, v in results.items() if k.startswith("ml_")}
gnn_graph_results = {k: v for k, v in results.items() if k.startswith("gnn_") and "graph_only" in k}
gnn_enriched_results = {k: v for k, v in results.items() if k.startswith("gnn_") and "enriched" in k}
ensemble_results = {k: v for k, v in results.items() if k.startswith("ensemble_")}

# Print ML Results
print("\n--- ML MODELS (Fingerprints Only) ---")
print(f"{'Model':<35} {'Features':<15} {'AUC':>6} {'ACC':>6} {'MCC':>6}")
print("-" * 75)
for name, m in sorted(ml_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
    parts = name.split("_")
    ml_type = parts[1].upper()
    feat_type = parts[2] if len(parts) > 2 else ""
    auc_str = f"{m['auc']:.4f}" if m.get("auc") is not None else "N/A"
    print(f"{ml_type:<35} {feat_type:<15} {auc_str:>6} {m['acc']:.4f} {m['mcc']:.4f}")

# Print GNN Graph Only Results
print("\n--- GNN MODELS - GRAPH ONLY (No Fingerprints) ---")
print(f"{'Model':<35} {'Input':<15} {'AUC':>6} {'ACC':>6} {'MCC':>6}")
print("-" * 75)
for name, m in sorted(gnn_graph_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
    parts = name.split("_")
    model_type = parts[1].upper()
    auc_str = f"{m['auc']:.4f}" if m.get("auc") is not None else "N/A"
    print(f"{model_type:<35} {'32-dim graph':<15} {auc_str:>6} {m['acc']:.4f} {m['mcc']:.4f}")

# Print GNN Enriched Results
print("\n--- GNN MODELS - ENRICHED GRAPHS (atom + Morgan + MACCS) ---")
print(f"{'Model':<35} {'Input':<15} {'AUC':>6} {'ACC':>6} {'MCC':>6}")
print("-" * 75)
for name, m in sorted(gnn_enriched_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
    parts = name.split("_")
    model_type = parts[1].upper()
    auc_str = f"{m['auc']:.4f}" if m.get("auc") is not None else "N/A"
    print(f"{model_type:<35} {'2246-dim enrich':<15} {auc_str:>6} {m['acc']:.4f} {m['mcc']:.4f}")

# Print Ensemble Results
print("\n--- ENSEMBLE MODELS (GNN + ML) ---")
print(f"{'Model':<35} {'Method':<15} {'AUC':>6} {'ACC':>6} {'MCC':>6}")
print("-" * 75)
for name, m in sorted(ensemble_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
    parts = name.split("_")
    method = f"{parts[1]}+{parts[2].upper()}"
    auc_str = f"{m['auc']:.4f}" if m.get("auc") is not None else "N/A"
    print(f"{'Ensemble':<35} {method:<15} {auc_str:>6} {m['acc']:.4f} {m['mcc']:.4f}")
...
# Calculate averages
graph_only_aucs = [v.get("auc", 0) for k, v in gnn_graph_results.items() if v.get("auc")]
enriched_aucs = [v.get("auc", 0) for k, v in gnn_enriched_results.items() if v.get("auc")]
maccs_aucs = [v.get("auc", 0) for k, v in ml_results.items() if "MACCS" in k and v.get("auc")]
morgan_aucs = [v.get("auc", 0) for k, v in ml_results.items() if "Morgan_" in k and "MACCS" not in k and v.get("auc")]
both_aucs = [v.get("auc", 0) for k, v in ml_results.items() if "Morgan+MACCS" in k and v.get("auc")]

if graph_only_aucs:
    print(f"  GNN (graph only) avg AUC:     {np.mean(graph_only_aucs):.4f}")
if enriched_aucs:
...
if graph_only_aucs and enriched_aucs:
    improvement = np.mean(enriched_aucs) - np.mean(graph_only_aucs)
    print(f"\n  Enriched vs Graph-only improvement: {improvement:+.4f} AUC")
    print(f"  WHY? Fingerprints inject 20 years of cheminformatics knowledge into GNN")

print("=" * 100)

# %%
# @title 11. VISUALIZATION (Optional - requires matplotlib)
try:
    ...
    for name, m in sorted(ml_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
        parts = name.split("_")
        label = f"ML-{parts[1].upper()}\n({parts[2]})"
        categories.append(label)
        aucs.append(m.get("auc", 0))
        colors.append("#2196F3")  # Blue

    # GNN graph only
    for name, m in sorted(gnn_graph_results.items(), key=lambda x: x[1].get("auc") or 0, reverse=True):
        parts = name.split("_")
        label = f"GNN-{parts[1].upper()}\n(graph)"
        categories.append(label)
        aucs.append(m.get("auc", 0))
    ...
    bars = ax.bar(range(len(categories)), aucs, color=colors, edgecolor="black", linewidth=0.5)

    ax.set_xticks(range(len(categories)))
    ax.set_xticklabels(categories, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel("AUC", fontsize=12)
    ax.set_title("VEGFR2: Feature Comparison Across All Models", fontsize=14)
    ax.set_ylim(0.5, 1.0)
    ax.axhline(y=0.7, color="red", linestyle="--", alpha=0.5, label="Random baseline")
    ...
    plt.show()
    print("Plot saved to feature_comparison.png")
...
output = {
    ...
    "results": {},
    "summary": {
        "graph_only_avg_auc": np.mean(graph_only_aucs) if graph_only_aucs else None,
        "enriched_avg_auc": np.mean(enriched_aucs) if enriched_aucs else None,
        "maccs_avg_auc": np.mean(maccs_aucs) if maccs_aucs else None,
        "morgan_avg_auc": np.mean(morgan_aucs) if morgan_aucs else None,
        "both_avg_auc": np.mean(both_aucs) if both_aucs else None,
    },
}
...
print(f"\nResults saved to feature_comparison_results.json")
