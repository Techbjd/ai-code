...
from torch_geometric.nn import GCNConv, GATConv, GATv2Conv, NNConv, global_mean_pool
...
class GATv2_PyG(nn.Module):
    """GATv2: Improved Graph Attention with dynamic attention mechanism.
    
    Key difference from GAT: GAT computes attention as:
...
def build_pyg_model(
    ...
    raise ValueError(f"Unknown model: {name}. Available: gcn, gat, gatv2, mpnn")
