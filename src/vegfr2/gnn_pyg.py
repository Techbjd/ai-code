...
class GCN_PyG(nn.Module):
    def __init__(self, in_dim: int = 32, hidden: int = 64, layers: int = 3, out_dim: int = 1):
        super().__init__()
...
class GAT_PyG(nn.Module):
    def __init__(self, in_dim: int = 32, hidden: int = 64, layers: int = 3, heads: int = 4, out_dim: int = 1):
        super().__init__()
        self.init_kwargs = {"in_dim": in_dim, "hidden": hidden, "layers": layers, "heads": heads, "out_dim": out_dim}
        self.convs = nn.ModuleList()
        self.convs.append(GATConv(in_dim, hidden // heads, heads=heads))
        for _ in range(layers - 2):
            self.convs.append(GATConv(hidden, hidden // heads, heads=heads))
        self.convs.append(GATConv(hidden, hidden, heads=1))  # last layer single head
        self.output = nn.Linear(hidden, out_dim)
...
class MPNN_PyG(nn.Module):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor, batch: Tensor) -> Tensor:
        ...
        h = x
        for edge_net, conv in zip(self.edge_nets, self.convs):
