...
class MPNN(nn.Module):
    def __init__(self, in_dim: int = 74, hidden: int = 64, layers: int = 3, out_dim: int = 1, edge_dim: int = 11, dropout: float = 0.2):
        super().__init__()
...
def train_gnn_model(
    ...
    model = build_model(name, in_dim=74, hidden=hidden, layers=layers, heads=heads, edge_dim=11).to(device)
