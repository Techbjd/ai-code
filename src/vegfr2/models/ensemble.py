...
class GNNEnsembleClassifier:
    ...
    def _build_gnn(self, in_dim: int = 74, edge_dim: int = 11) -> nn.Module:
    ...
    def fit(
        ...
        self.gnn_model = self._build_gnn(in_dim=74).to(device)
    ...
    def load(cls, path: str, device: str | torch.device = "cpu") -> "GNNEnsembleClassifier":
        ...
        if state["gnn_state"] is not None:
            ensemble.gnn_model = ensemble._build_gnn(in_dim=74).to(device)
