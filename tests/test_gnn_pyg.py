...
def test_pyg_model_forward(name, enriched_batch):
    """PyG models produce [B, 1] logits with enriched features."""
    enriched_dim = get_enriched_node_dim(use_morgan=True, use_maccs=True)
    model = build_pyg_model(name, in_dim=enriched_dim, hidden=32, layers=2, heads=2)
    model.eval()
...
@pytest.mark.parametrize("name", ["gcn", "gat", "gatv2", "mpnn"])
def test_pyg_model_backward(name, enriched_batch):
    """PyG models have finite gradients with enriched features."""
    enriched_dim = get_enriched_node_dim(use_morgan=True, use_maccs=True)
    model = build_pyg_model(name, in_dim=enriched_dim, hidden=32, layers=2, heads=2)
    model.train()
...
def test_gatv2_vs_gat_different():
    ...
    gat = build_pyg_model("gat", in_dim=enriched_dim, hidden=32, layers=2, heads=2)
    torch.manual_seed(42)
    gatv2 = build_pyg_model("gatv2", in_dim=enriched_dim, hidden=32, layers=2, heads=2)
    
    dataset = EnrichedPyGDataset(["CCO", "c1ccccc1"], [1, 0])
